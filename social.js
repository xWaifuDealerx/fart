...
`;
    ...
    share.innerHTML = '<h3 id="fwShareTtl">Milestone!</h3><div class="lab" id="fwShareLab">You did it.</div><button class="btn-tweet" id="fwShareGo">\u{1D54F} Share on X</button><button class="btn-skip" id="fwShareSkip">Skip <kbd class="kb">Enter</kbd></button>';
    document.body.appendChild(share);
    function skipShare(){ share.classList.remove('show'); }
    document.getElementById('fwShareSkip').addEventListener('click', skipShare);
    // pressing Enter while the milestone popup is open skips it
    window.addEventListener('keydown', (e) => {
      if((e.code === 'Enter' || e.code === 'NumpadEnter') && share.classList.contains('show')){ e.preventDefault(); e.stopPropagation(); skipShare(); }
    }, true);
